...
export function parseManualServerConfig(userInput: string): ManualServerConfig {
  // Trim leading and trailing whitespace, for URLs and imperfectly positioned cursor.
  userInput = userInput.trim();

  // Is test an outlinem:// URL? We do not use URL since in some environments it actually will
  // construct a (very weird) URL from a valid JSON blob - and even when it works the relevant field
  // includes a leading "://".
  if (userInput.startsWith('outlinem://')) {
    userInput = decodeURIComponent(userInput.substring('outlinem://'.length));
  }

  return parseManualServerConfigJson(userInput);
}

// Parses a server management URL (generated by the server install script) leniently.
// TODO: ignore case of key names
// TODO: check value types
export function parseManualServerConfigJson(userInput: string): ManualServerConfig {
