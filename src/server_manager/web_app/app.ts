...
export class App {
  ...
  constructor(
    ...
    appRoot.addEventListener('ShowServerRequested', async (event: CustomEvent) => {
      ...
      if (manualServer) {
        ...
        this.showServerIfHealthy(manualServer, displayServer);
      } else {
  ...
  async start(): Promise<void> {
    ...
    this.loadManualServers();
    await this.loadDigitalOceanServers();
    this.maybeShowLastDisplayedServer();
  }

  private async loadManualServers(): Promise<void> {
    const servers = await this.manualServerRepository.listServers();
    const displayServers: DisplayServer[] = await Promise.all(servers.map(makeDisplayServer));
    this.appRoot.manualServerList = displayServers;
  }
  ...
  private async loadDigitalOceanServers(): Promise<void> {
    ...
    await this.syncServersToDisplay(installedManagedServers);
  }
  ...
  private async getServerFromRepository(displayServer: DisplayServer): Promise<server.Server|null> {
    ...
    if (displayServer.isManaged) {
      const digitalOceanAccount = this.accountRepository.getDigitalOceanAccount();
      if (!!digitalOceanAccount) {
        ...
        const managedServers = await digitalOceanAccount.listServers(false);
  ...
  private maybeShowLastDisplayedServer(): void {
    if (!!this.serverBeingCreated) {
  ...
  private async showServerIfHealthy(server: server.Server, displayServer: DisplayServer):
    ...
    if (isHealthy) {
      ...
      if (isManagedServer(server)) {
        // TODO: We only sync managed servers atm because the manual servers
        //  are not in the display repository.
        await this.syncServerToDisplay(server);
      }
  ...
  private showServerUnreachable(server: server.Server, displayServer: DisplayServer) {
    ...
    serverView.retryDisplayingServer = () => {
      ...
      const digitalOceanAccount = this.accountRepository.getDigitalOceanAccount();
      if (serverView.isServerManaged && !!digitalOceanAccount) {
        serverExistsPromise = digitalOceanAccount.listServers().then((managedServers) => {
  ...
  private connectToDigitalOcean() {
    const digitalOceanAccount = this.accountRepository.getDigitalOceanAccount();
    if (digitalOceanAccount) {
      this.enterDigitalOceanMode(digitalOceanAccount).then((managedServers) => {
        if (!!this.serverBeingCreated) {
    ...
    session.result
        .then(async (accessToken) => {
          ...
          await this.accountRepository.connectDigitalOceanAccount(accessToken);
          const digitalOceanAccount = this.accountRepository.getDigitalOceanAccount();
  ...
  private showCreateServer() {
    ...
    this.digitalOceanRetry(() => {
          return this.accountRepository.getDigitalOceanAccount().getRegionMap();
        })
        .then(
            (map) => {
  ...
  public createDigitalOceanServer(regionId: server.RegionId) {
    ...
    return this
        .digitalOceanRetry(() => {
          return this.accountRepository.getDigitalOceanAccount().createServer(regionId, serverName);
        })
        .then((server) => {
